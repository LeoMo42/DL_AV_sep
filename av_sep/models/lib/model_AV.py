...
from keras.layers import BatchNormalization, ReLU, Reshape, Lambda
...
def AV_model(people_num=2):
    ...
    # # --------------------------- VS start ---------------------------
    # video_input = Input(shape=(75, 1, 1024))
    # print('vs_0:', video_input.shape)
    # vs_conv1 = Convolution2D(256, kernel_size=(7, 1), strides=(1, 1), padding='same', dilation_rate=(1, 1), name='vs_conv1')(video_input)
    # vs_conv1 = BatchNormalization()(vs_conv1)
    # vs_conv1 = ReLU()(vs_conv1)
    # print('vs_1:', vs_conv1.shape)
    #
    # vs_conv2 = Convolution2D(256, kernel_size=(5, 1), strides=(1, 1), padding='same', dilation_rate=(1, 1), name='vs_conv2')(vs_conv1)
    # vs_conv2 = BatchNormalization()(vs_conv2)
    # vs_conv2 = ReLU()(vs_conv2)
    # print('vs_2:', vs_conv2.shape)
    #
    # vs_conv3 = Convolution2D(256, kernel_size=(5, 1), strides=(1, 1), padding='same', dilation_rate=(2, 1), name='vs_conv3')(vs_conv2)
    # vs_conv3 = BatchNormalization()(vs_conv3)
    # vs_conv3 = ReLU()(vs_conv3)
    # print('vs_3:', vs_conv3.shape)
    #
    # vs_conv4 = Convolution2D(256, kernel_size=(5, 1), strides=(1, 1), padding='same', dilation_rate=(4, 1), name='vs_conv4')(vs_conv3)
    # vs_conv4 = BatchNormalization()(vs_conv4)
    # vs_conv4 = ReLU()(vs_conv4)
    # print('vs_4:', vs_conv4.shape)
    #
    # vs_conv5 = Convolution2D(256, kernel_size=(5, 1), strides=(1, 1), padding='same', dilation_rate=(8, 1), name='vs_conv5')(vs_conv4)
    # vs_conv5 = BatchNormalization()(vs_conv5)
    # vs_conv5 = ReLU()(vs_conv5)
    # print('vs_5:', vs_conv5.shape)
    #
    # vs_conv6 = Convolution2D(256, kernel_size=(5, 1), strides=(1, 1), padding='same', dilation_rate=(16, 1), name='vs_conv6')(vs_conv5)
    # vs_conv6 = BatchNormalization()(vs_conv6)
    # vs_conv6 = ReLU()(vs_conv6)
    # print('vs_6:', vs_conv6.shape)
    #
    # vs_out = Reshape((75, 256, 1))(vs_conv6)
    # print('vs_out:', vs_out.shape)
    #
    # VS_out = UpSampling2DBilinear((298, 256))(vs_out)
    # # VS_out = Deconvolution2D()
    # print('VS_out:', VS_out.shape)
    #
    # VS_out = Reshape((298, 256))(VS_out)
    # print('VS_out_new:', VS_out.shape)
    # # --------------------------- VS end ---------------------------
    ...
    video_input = Input(shape=(75, 1, 1024, people_num))
...
if __name__ == '__main__':
    ...
    video_input = np.random.rand(5, 75, 1, 1024, people_num)     # 5 audio parts, (75, 1, 1024) feature, people num to be defined
    ...
    filepath = path + "/AVmodel-" + str(people_num) + "p-{epoch:02d}-{val_loss:.4f}.h5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='min')

    AV_model = AV_model(people_num)
    AV_model.fit([audio_input, video_input], audio_label,
                 epochs=5,
                 batch_size=2,
                 validation_data=([audio_input, video_input], audio_label),
                 shuffle=False,
                 callbacks=[TensorBoard(log_dir='./log_AV'), checkpoint])

    # AV_model = load_model('./saved_models_AV/***.h5')     # this line shows how to load model
    # AV_model.predict([audio_input, video_input])
